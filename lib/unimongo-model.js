...
class UnimongoModel extends SchemaModel {
	...
	_makeIntervalExpression(interval, field) {
		interval = +interval;

		if (_.isNaN(interval)) {
			throw new XError(XError.BAD_REQUEST, 'Aggregate interval must be number', { interval });
		}
	...
	_makeGroupExpression(spec) {
		...
		let groupValueSpecs = groupBy.map((groupBySpec) => {
			...
			if (groupBySpec.ranges) {
				valueExpr = this._makeRangesExpression(groupBySpec.ranges, field);
			} else if (groupBySpec.dateRanges) {
				valueExpr = this._makeRangesExpression(groupBySpec.dateRanges, field);
			} else if (groupBySpec.interval) {
				valueExpr = this._makeIntervalExpression(groupBySpec.interval, field);
			} else if (groupBySpec.timeInterval) {
				valueExpr = this._makeTimeIntervalExpression(groupBySpec.timeInterval, field);
			} else {
				valueExpr = '$' + field;
			}
