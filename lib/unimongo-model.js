	...
	_makeGroupExpression(spec) {
		...
		let groupValueSpecs = groupBy.map((groupBySpec) => {
			...
			if (groupBySpec.ranges) {
				...
				valueExpr = this._makeTimeIntervalExpression(field, groupBySpec.timeInterval);
			} else {
				valueExpr = '$' + field;
			}
					...
					if (groupBy) {
						...
						for (let keyIndex = 0; keyIndex < groupBy.length; keyIndex++) {
							...
							if (keyValue === undefined || keyValue === null) {
								...
								typeof groupBy[keyIndex] === 'string' ||
								groupBy[keyIndex].ranges ||
								groupBy[keyIndex].interval ||
								groupBy[keyIndex].dateRanges
							) {
								keyArray.push(keyValue);
							} else if (groupBy[keyIndex].timeInterval) {
