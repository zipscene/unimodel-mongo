...
class UnimongoModel extends SchemaModel {
	...
	constructor(modelName, schema, db, options = {}) {
		...
		this.collectionOptions = UnimongoModel._transformMongoOptions(options, this.db);
	 ...
	 * @param {UnimongoDb} [db] - Database instance to use for certain property checks such as `readPreference`
	 * @since v0.1.0
	 */
	...
	find(query, options = {}) {
		...
		let cursor;
		return this.collectionPromise
			.then((collection) => UnimongoModel._findWithOptions(collection, query.getData(), options))
			.then((mongoCursor) => {
				cursor = mongoCursor;
	...
	insertMulti(datas, options = {}) {
		...
		let mongoOptions = UnimongoModel._transformMongoOptions(options, this.db);
