...
const toNumberExpr = function(expr, count, base) {
	if (count === 1) return expr;

	return {
		$subtract: [
			expr,
			{ $mod: [ expr, (typeof base !== undefined) ? base : count ] }
		]
	};
};
...
class UnimongoModel extends SchemaModel {
	...
	// Accepts time intervals in the form "3d" or "2s"
	// Valid suffixes are second/s, minute/m, hour/h, day/d, week/w, month/M, year/y, quarter/q
	// The output values are truncated ISO 8601 timestamps of the beginning of each range.
	// Unused fields at the end are truncated.
	_makeTimeComponentExpression(field, interval, count = 1) {
		let $field = '$' + field;

		if (typeof interval !== 'string') {
			throw new XError(XError.BAD_REQUEST, 'Aggregate time interval must be string', { interval });
		}
			...
			case 'second':
				return {
					$concat: [
						...
						toStringExpr(toNumberExpr({ $second: $field }, count))
					]
				};
			case 'minute':
				return {
					$concat: [
						...
						toStringExpr({ $dayOfMonth: $field }), 'T',
						toStringExpr({ $hour: $field }), ':',
						toStringExpr(toNumberExpr({ $minute: $field }, count))
					]
				};
			case 'hour':
				return {
					$concat: [
						...
						toStringExpr({ $month: $field }), '-',
						toStringExpr({ $dayOfMonth: $field }), 'T',
						toStringExpr(toNumberExpr({ $hour: $field }, count))
					]
				};
			case 'day':
				return {
					$concat: [
						toStringExpr({ $year: $field }), '-',
						toStringExpr({ $month: $field }), '-',
						toStringExpr(toNumberExpr({ $dayOfMonth: $field }, count))
					]
				};
			case 'week':
				return {
					$concat: [
						toStringExpr({ $year: $field }), '-',
						toStringExpr({ $month: $field }), '-',
						toStringExpr(toNumberExpr(toNumberExpr({ $dayOfMonth: $field }, count, 7), count))
					]
				};
			case 'month':
				return {
					$concat: [
						toStringExpr({ $year: $field }), '-',
						toStringExpr(toNumberExpr({ $month: $field }, count))
					]
				};
			case 'quarter':
				return {
					$concat: [
						toStringExpr({ $year: $field }), '-',
						toStringExpr(toNumberExpr(toNumberExpr({ $month: $field }, count, 4), count))
					]
				};
			case 'year':
				return toStringExpr(toNumberExpr({ $year: $field }, count));
			default:
				throw new XError(XError.BAD_REQUEST, 'Invalid aggregate time interval', { interval });
		}
	}
	...
	_makeGroupExpression(spec) {
		...
		let groupValueSpecs = groupBy.map((groupBySpec) => {
			...
			if (groupBySpec.ranges) {
				...
				valueExpr = this._makeRangesExpression(field, groupBySpec.dateRanges);
				...
				valueExpr = this._makeTimeComponentExpression(field, groupBySpec.timeComponent, groupBySpec.timeComponentCount);
			} else {
				valueExpr = '$' + field;
			}
