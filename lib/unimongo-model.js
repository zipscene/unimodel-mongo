				...
				for(let resultKey in resultEntry) {
					...
					if (matches) {
						...
						if (!resultsByAggregate[matchGroup]) {
							resultsByAggregate[matchGroup] = {};
							if (!isKey) resultsByAggregate[matchGroup].stats = {};
						}
				...
				for(let aggregateName in resultsByAggregate) {
					...
					if (groupBy) {
						...
						for(let keyIndex = 0; keyIndex < groupBy.length; keyIndex++) {
							let keyValue = aggrResultEntry['key_' + keyIndex];
							delete aggrResultEntry['key_' + keyIndex];
