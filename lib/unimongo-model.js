 ...
 * @param {Number} [base=0] - Number to start the interval sequence at
 * @param {Number} [offset=0] - Number to offset the first interval by
 * @return {Object} - A MongoDB aggregate expression
 * @since v0.1.0
 */
const toIntervalExpr = (expr, interval = 1, base = 0, offset = 0) => {
	...
	if (base !== 0) {
		// Align the base parameter to a range that makes sense and functions as expected
 ...
 * @param {String|Object} ...values - Values or expressions which make up an ISO 8601 time string when combined
 * @return {Object} - A MongoDB aggregate expression
 * @since v0.1.0
 */
const fillRemainingTimeComponents = (...values) => {
	if (values.length < 1) values.push('1970');
	if (values.length < 2) values.push('-');
	if (values.length < 3) values.push('01');
	if (values.length < 4) values.push('-');
	if (values.length < 5) values.push('01');
	if (values.length < 6) values.push('T');
	if (values.length < 7) values.push('00');
	if (values.length < 8) values.push(':');
	if (values.length < 9) values.push('00');
	if (values.length < 10) values.push(':');
	if (values.length < 11) values.push('00');
	if (values.length < 12) values.push('Z');

	return values;
};

/**
 * Sort aggregate results by string representation of keys
 *
 * @method keySort
 * @param {Object} a
 * @param {Object} b
...
class UnimongoModel extends SchemaModel {
	...
	/**
	 * The output values are ISO 8601 timestamps of the beginning of each range.
	 *
	 * @method _makeTimeComponentExpression
	 * @private
	 * @param {String|Object} field - A MongoDB field expression
	 * @param {String} unit
	 * @param {Number} interval
	 * @return {Object} - A MongoDB aggregate expression
	 * @since v0.1.0
	 */
	_makeTimeComponentExpression(field, unit, interval = 1) {
		let $field = '$' + field;

		if (typeof unit !== 'string') {
			throw new XError(XError.BAD_REQUEST, 'Aggregate time unit must be string', { unit });
		}
		...
		switch (unit) {
				...
				case 'second':
					...
					break;
				default:
					throw new XError(XError.BAD_REQUEST, 'Invalid aggregate time unit', { unit });
		}
	 ...
	 * returns an expression that will result in the start value of the interval in which the field lies.
	 *
	 * @method _makeRangesExpression
	 * @private
	 * @param {String|Object} field - A MongoDB field expression
	 * @param {Array{*}} ranges
	 * @return {Object} - A MongoDB aggregate expression
	 * @since v0.1.0
	 */
	_makeIntervalExpression(field, interval, base = 0) {
		if (typeof interval === 'string' && interval[0] === 'P') {
			interval = moment.duration(interval);

			let fieldType = this.schema.getSubschemaData(field).type;
			if (fieldType === 'date') {
				throw new XError(
		...
		if (_.isNaN(interval)) {
			throw new XError(XError.BAD_REQUEST, 'Aggregate interval must be numeric.', { interval });
		}
	 ...
	 * @since v0.1.0
	 */
	_makeRangesExpression(field, ranges) {
		if (!Array.isArray(ranges)) throw new XError(XError.BAD_REQUEST, 'Aggregate ranges must be array', { ranges });
	 ...
	 *
	 * @method _makeGroupExpression
	 * @private
	 * @param {commonQuery.Aggregate} aggregate
	 * @return {Object}
	 * @since v0.1.0
	 */
	_makeGroupExpression(aggregate) {
		let aggregateData = aggregate.getData();

		if (!_.has(aggregateData, 'groupBy')) {
			if (!_.has(aggregateData, 'stats')) {
		...
		let groupValueSpecs = groupBy.map((groupBySpec) => {
			...
			if (groupBySpec.ranges) {
				valueExpr = this._makeRangesExpression(field, groupBySpec.ranges);
			} else if (groupBySpec.interval) {
				valueExpr = this._makeIntervalExpression(field, groupBySpec.interval, groupBySpec.base);
			} else if (groupBySpec.timeComponent) {
				valueExpr = this._makeTimeComponentExpression(
					field,
	 ...
	 *
	 * @method _createAggregatePipelines
	 * @private
	 ...
	 * @since v0.1.0
	 */
	_createAggregatePipelines(query, aggregates) {
		let aggregatesByGroup = {};
		let queryData = query.getData();

		_.forEach(aggregates, (aggregate, index) => {
			let groupExpression = this._makeGroupExpression(aggregate);
			...
			groupExpressionData.specIndices.forEach((index) => {
				...
				if (aggregateSpec.stats) {
					_.forEach(aggregateSpec.stats, (stat, field) => {
						if (stat.count) {
							fields[`agg_${index}_count_${field}`] = {
								$sum: {
									$cond: [
										{ $eq: [
											{ $ifNull: [ `$${field}`, null ] },
						...
						if (stat.avg) fields[`agg_${index}_avg_${field}`] = { $avg: `$${field}` };
						if (stat.min) fields[`agg_${index}_min_${field}`] = { $min: `$${field}` };
						if (stat.max) fields[`agg_${index}_max_${field}`] = { $max: `$${field}` };
					});
				}
	 ...
	 * @since v0.1.0
	 */
	_createAggregateResult(pipelines, aggregates) {
				...
				for (let resultKey in resultEntry) {
					...
					if (matches) {
						...
						if (matchOperator === 'key') {
							hasStats = false;
							matchOperator = `${matchOperator}_${matchField}`;
							matchField = null;
						}
	...
	aggregateMulti(query, aggregates, options = {}) {
		...
		if (!_.isFunction(query.normalize)) query = commonQuery.createQuery(query);
		_.forEach(aggregates, (aggregate, key) => {
			if (!_.isFunction(aggregate.normalize)) aggregates[key] = commonQuery.createAggregate(aggregate);
		});

		query.normalize({ schema: this.schema });

		let pipelines = this._createAggregatePipelines(query, aggregates);
		...
		return this.collectionPromise
			.then((collection) => {
				...
				return Promise.all(results)
					.then((pipelines) => {
						return this._createAggregateResult(pipelines, aggregates);
					});
			});
	}
