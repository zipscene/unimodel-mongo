			...
			groupExpressionData.specIndices.forEach((aggregateIndex) => {
				...
				if (aggregateSpec.stats) {
					_.forEach(aggregateSpec.stats, (stat, statsField) => {
						if (stat.count) {
							aggregateFields[`agg_${aggregateIndex}_count`] = {
								$sum: {
									$cond: [
										{
											$eq: [
						...
						if (stat.avg) aggregateFields[`agg_${aggregateIndex}_avg`] = { $avg: `$${statsField}` };
						if (stat.min) aggregateFields[`agg_${aggregateIndex}_min`] = { $min: `$${statsField}` };
						if (stat.max) aggregateFields[`agg_${aggregateIndex}_max`] = { $max: `$${statsField}` };
					});
				}
		...
		pipelines.forEach((pipelineData) => {
			...
			for(let resultEntryIdx = 0; resultEntryIdx < results.length; resultEntryIdx++) {
				...
				for(let resultKey in resultEntry) {
					matches = /^agg_([A-Za-z0-9-]+)_(.*)$/.exec(resultKey);
					if (matches) {
						aggrNum = matches[1];
						if (!resultsByAggregate[aggrNum]) resultsByAggregate[aggrNum] = {};
						resultsByAggregate[aggrNum][matches[2]] = resultEntry[resultKey];
					}
				}

				// Reformat the key fields in each entry according to the type of grouping requested
				for(aggrNum in resultsByAggregate) {
					let aggrResultEntry = resultsByAggregate[aggrNum];
					let aggrSpec = specs[aggrNum];
					...
					if (groupBy) {
						...
						if (keyArray.length === 1) {
							aggrResultEntry.key = keyArray[0];
						} else if (keyArray.length) {
							aggrResultEntry.key = keyArray;
						}
					}

					if (!aggregateResults[aggrNum]) aggregateResults[aggrNum] = [];
					aggregateResults[aggrNum].push(aggrResultEntry);
				}
			}
		});
