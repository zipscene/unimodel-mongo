...
class MongoModel extends SchemaModel {
	...
	static _transformMongoOptions(options, db) {
		...
		for (let option in options) {
			if (option in mongoOptionMap) {
				...
				let mongoOption = options[option];

				// If a timeout is set, convert it from seconds to milliseconds.
				if (option === 'timeout') {
					mongoOption = mongoOption * 1000;
				}

				res[mongoOptionMap[option]] =  mongoOption;
			}
		}
	...
	static _findWithOptions(collection, query, options) {
		...
		if (_.isNumber(options.timeout)) {
			cursor.maxTimeMS(options.timeout * 1000);
		}
	...
	count(query, options = {}) {
		...
		return this.collectionPromise
			.then((collection) => collection.find(query.getData(), mongoOptions).count())
			.catch((err) => {
				throw MongoError.fromMongoError(err);
			});
	}
		...
		return this.collectionPromise
			.then((collection) => {
				let results = pipelines.map((pipelineData) => {
					let mongoOptions = MongoModel._transformMongoOptions(options);

					return collection.aggregate(pipelineData.pipeline, mongoOptions).toArray()
