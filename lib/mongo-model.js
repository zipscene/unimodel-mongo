...
class MongoModel extends SchemaModel {
	...
	/**
	 * Translates a query that contains a $near operator into a geoNear command and returns
	 * the results.  Parameters are the same as for `_findWithOptions`.
	 *
	 * @method _translatedGeonear
	 * @static
	 ...
	 */
	static _translatedGeonear(collection, query, field, nearParams, model, options) {
		// We can only translate $near to a geoNear command if there's a single $near in the query
		// and if it's AND'd with the rest of the query.
		let near = nearParams.$geometry || nearParams;
		let minDistance = nearParams.$minDistance;
		let maxDistance = nearParams.$maxDistance;

		// Check to make sure there's an index on the field
		let indexes = model.getIndexes();
		let hasProperIndex = false;
		let indexType;
		let num2dIndexes = 0;
		for (let { spec: index } of indexes) {
			if (index[field] === '2dsphere' || index[field] === '2d') {
		...
		if (!hasProperIndex) {
			throw new XError(XError.INVALID_ARGUMENT, 'No suitable index for for $near');
		}
		if (num2dIndexes > 1) throw new XError(XError.INTERNAL_ERROR, 'There must be only 1 2d index');

		// Convert maxDistance and minDistance from meters to radians if meters are expected from the user
		if (indexType !== '2d') {
			const earthCircumferenceMeters = 40075000;
			if (_.isNumber(maxDistance)) {
				maxDistance = maxDistance / earthCircumferenceMeters * 2 * Math.PI;
			}
			if (_.isNumber(minDistance)) {
				minDistance = minDistance / earthCircumferenceMeters * 2 * Math.PI;
			}
		}

		// Remove the $near from the query
		query._traverse({
		...
		let x = objtools.getPath(near, 'coordinates.0');
		let y = objtools.getPath(near, 'coordinates.1');
		if (!x || !y) throw new XError(XError.INVALID_ARGUMENT, 'Could not extract coordinates from $near');
		return Promise.resolve()
			.then(() => {
				let opts = {
					readPreference: options.readPreference,
					num: _.isNumber(options.limit) ? options.limit : 1000,
					minDistance,
					maxDistance,
					query: query.getData(),
					spherical: indexType !== '2d',
					includeLocs: false
				};
				for (let key in opts) {
					if (opts[key] === undefined) delete opts[key];
				}
				return collection.geoNear(x, y, opts);
			})
			.then((result) => {
				let datas = _.map(result.results, 'obj');
				return MongoModel._manualApplyQueryOptions(datas, options);
			});
	}
	...
	static _findWithOptions(collection, query, model, options) {
		...
		if (typeof cQuery.getOperators !== 'function') {
			cQuery = createQuery(objtools.deepCopy(query));
		}
		if (_.includes(cQuery.getOperators(), '$near')) {
			...
			if (!nearField || !nearParams) {
				throw new XError(XError.INTERNAL_ERROR, 'Tried to translate to geonear without a $near');
			}
			...
			if (!nearIsAnded) throw new XError(XError.INVALID_ARGUMENT, '$near can only be used inside $and');
			...
			// Translate to geoNear if this is a possibly sharded collection
			if (!model.options.neverSharded) {
				return MongoModel._translatedGeonear(collection, cQuery, nearField, nearParams, model, options);
			}
	...
	aggregateMulti(query, aggregates, options = {}) {
		...
		return this.collectionPromise
			.then((collection) => {
				...
				pipelineData = aggregateUtils.createAggregatePipeline(this.schema, query, aggregates, options);
				return collection;
			})
			.then((collection) => {
				if (_.isNumber(options.timeout)) {
				...
				return cursor.toArray()
					...
					.then((resultDocs) => {
						pipelineData.results = _(resultDocs)
							.map(_.values)
							.flattenDeep()
							.value();

						return aggregateUtils.createAggregateResult(this.schema, pipelineData, aggregates);
					});
			})
			.then((results) => {
	...
	update(query, update, options = {}) {
		let prof = this.profiler.begin('#update');
		...
		if ((!isUpdatingMap && !isUpdatingGeoHashed) || options.forceAtomic) {
			...
			return this.findStream(query, mongoOptions).each((doc) => {
				...
				.then(() => {
					...
					if (numUpdated > 0 || !options.upsert) return;
					let newDocData = {};
