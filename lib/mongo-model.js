...
class MongoModel extends SchemaModel {
	...
	normalizeQuery(query, options) {
		// `normalizeQuery` could potentially introduce new empty $or expressions but
		// cannot handle them, so call _normalizeEmptyOr both before and after

		query = this._normalizeEmptyOr(query);
	...
	/**
	 * Empty $or is a valid match-nothing common-query query but is not a valid mongo query;
	 * it will be normalized out when nested but needs to be handled at the root level
	 *
	 * @method _normalizeEmptyOr
	 * @param {Object} data - Raw query data to normalize
	 * @since v1.9.0
	 */
	_normalizeEmptyOr(query) {
		if (_.isArray(query.$or) && query.$or.length === 0) {
			if (!_.isArray(query.$and)) { query.$and = []; }
			query.$and.push({ _id: { $exists: true } });
