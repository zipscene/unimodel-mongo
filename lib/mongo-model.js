...
class MongoModel extends SchemaModel {
	...
	/**
	 * Internal function to remove all indexes on a collection that do not exist in the schema.
	 *
		...
		return collection.indexes()
			.then(indexes => {
				return pasync.eachSeries(indexes, (index) => {
					if (!this._shouldHaveIndex(index)) {
	 ...
	 */
	_shouldHaveIndex(index) {
		return _.some(this.getIndexes(), (schemaIndex) => _.isEqual(schemaIndex.spec, index.key));
	}

	/**
	 * Internal function to synchronize indexes on a collection with those in the schema.
	 *
	 ...
	 * @return {Promise} - Resolve with collection or rejects with XError
	 */
	synchronizeIndexes(collection) {
		return this.ensureIndexes(collection)
			.then(() => this.removeIndexes(collection));
	}
