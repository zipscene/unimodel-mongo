...
class MongoModel extends SchemaModel {
	...
	index(spec, options = {}) {
		...
		for (let path in spec) {
			let { spec: pathSpec } = this._convertIndex(path, spec[path]);
			let convertedPath = _.keys(pathSpec)[0];
			if (!/^_mapidx_/.test(convertedPath)) {
				// This is a normal path don't do anything special
				entry.spec[convertedPath] = pathSpec[convertedPath];
				continue;
			}
			// Cut off the '_mapidx_'
			let fullPath = convertedPath.substring(8);
			// Get array of fields
			let fullPathArray = fullPath.split('_');
			// Cut off the last '|' entry to get the field name
			let field = _.last(fullPathArray);
			// Join the path back to get the 'map path'
			let mapPath = _.initial(fullPathArray);
			// Add the field to the map set
			if (!mapSets[mapPath]) { mapSets[mapPath] = []; }
			mapSets[mapPath].push(field);
		}

		// Handle any map indicies
		if (!_.isEmpty(mapSets)) {
			if (_.size(mapSets) > 1) {
				// This will cause an NxN index
				throw new MongoError('Cannot index accross maps multiple maps.');
			}
			for (let mapPath in mapSets) {
				// Concatinate all mapPath/field combinations
	 ...
	 * @since v1.6.0
	 */
	_convertIndex(path, value, subschema) {
		let index = MongoModel._convertIndexType(value, subschema);
		let specPath = path;
		if (this.getSchema().hasParentType(path, 'map')) {
			...
			let mapPath = _.initial(parts).join('|');
			let field = _.last(parts);
			specPath = `_mapidx_${mapPath}_${field}`;
		}
		let indexEntry = {
			spec: {
				[`${specPath}`]: index
	...
	_getSchemaIndices() {
		...
		this.schema.traverseSchema({
			onSubschema(subschema, path/*, subschemaType*/) {
				...
				let indexEntry = model._convertIndex(path, subschema.index || true, subschema);
