...
const { coreDimensions: { LongLatDimension } } = require('zs-polytoken');
...
class MongoModel extends SchemaModel {
	 ...
	 */
	static _translatedGeonear(collection, query, model, options) {
		...
		let maxDistance;
		query._traverse({
			exprOperator(exprValue, _field, operator) {
				if (operator === '$near') {
					if (field) {
						throw new XError(XError.INVALID_ARGUMENT, '$near can only be used once per query');
					}
					field = _field;
					near = exprValue.$geometry || exprValue;
					minDistance = exprValue.$minDistance;
					maxDistance = exprValue.$maxDistance;
				}
			}
		});
		if (!field || !near) throw new XError(XError.INTERNAL_ERROR, 'Tried to translate to geonear without a $near');
		...
		return Promise.resolve()
			...
			.then((result) => {
				...
				if (_.isNumber(options.skip)) {
					datas = datas.slice(options.skip);
				}
	...
	static _findWithOptions(collection, query, model, options) {
		// If the query contains a $near, and we are operating in a potentially sharded environment, use
		// the geoNear command instead.  Mongo doesn't support $near in sharded environments.
		if (!model.options.neverSharded) {
			...
			if (_.includes(cQuery.getOperators(), '$near')) {
				return MongoModel._translatedGeonear(collection, cQuery, model, options);
			}
		}
	...
	update(query, update, options = {}) {
		...
		if (!isUpdatingMap || options.forceAtomic) {
	...
	normalizeDocumentIndexedGeoHashedValues(data) {
		...
		this.schema.traverse(data, {
			onField: (field, value, subschema) => {
				if (subschema.type !== 'geopoint') return;
