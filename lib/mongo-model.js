...
class MongoModel extends SchemaModel {
	...
	static createMapFieldHash(...fields) {
		let hash = MongoModel.createFieldHash(...fields);
	...
	index(spec, options = {}) {
		...
		if (!_.isEmpty(mapSets)) {
			...
			let specPath = MongoModel.createMapFieldHash(mapPath);
	...
	_convertIndexSpec(path, value, subschema) {
		...
		if (this.getSchema().hasParentType(path, 'map')) {
			...
			result.specPath = MongoModel.createMapFieldHash(path);
	...
	_normalizeQueryMapFields(data) {
		...
		let hash = MongoModel.createMapFieldHash(specPath);
