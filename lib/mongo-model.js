...
const profiler = new Profiler('MongoModel');
...
class MongoModel extends SchemaModel {
	 ...
	 */
	ensureIndexes() {
		let prof = profiler.begin('#ensureIndexes');
	 ...
	 */
	removeIndexes() {
		let prof = profiler.begin('#removeIndexes');
		return this.collectionPromise
	 ...
	 */
	synchronizeIndexes() {
		let prof = profiler.begin('#synchronizeIndexes');
	...
	initCollection() {
		...
		let prof = profiler.begin('#initCollection');
	...
	_createExisting(data = {}, options = {}) {
		let prof = profiler.begin('#_createExisting');
	...
	find(query, options = {}) {
		...
		return this.collectionPromise
			.then((collection) => {
				...
				profMongo = profiler.begin('#find mongo');
				return collection;
			})
	...
	findStream(query, options = {}) {
		...
		this.collectionPromise
			...
			.then((collection) => {
				let cursor = MongoModel._findWithOptions(collection, query.getData(), this, options);
