	...
	_fillSchemaIndices() {
		...
		this.schema.traverseSchema({
			onSubschema: (subschema, path/*, subschemaType*/) => {
				...
				let spec = subschema.index;
				if (!_.isObject(spec)) spec = { [path]: spec };
				...
				if (_.contains(path, '.')) {
					...
					for (let specKey in spec) {
						...
						spec[`${basePath}.${specKey}`] = spec[specKey];
						delete spec[specKey];
					}
				}

				if (_.keys(spec)[0] !== path) throw new XError(XError.INVALID_ARGUMENT, 'Field is not first index.');

				let options = {};
				if (subschema.sparse) options.sparse = true;
				if (subschema.unique) options.unique = true;

				this.index(spec, options);
			}
		}, {
