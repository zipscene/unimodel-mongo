...
class MongoModel extends SchemaModel {
	...
	initCollection() {
		...
		this._indexes = deduplicateIndexes(this._indexes);
...
// helper functions for #initCollection
function deduplicateIndexes(indexes) {
	return _.reject(indexes, isDuplicateIndex);
	function isDuplicateIndex(potentialDuplicateIndex) {
		let {
