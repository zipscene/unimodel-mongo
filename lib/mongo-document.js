...
class MongoDocument extends SchemaDocument {
	...
	constructor(model, data, options = {}) {
		const profSuper = model.profiler.begin('> super');
		super(model, data);
		profSuper.end();
		...
		const profPrepare = this.profiler.begin('@ _prepareNormalizeData');
		...
		if (this.options.isExisting) {
			const profCopy = this.profiler.begin('> deepCopy');
		...
		const profTrigger = this.profiler.begin('> triggerSync:post-init');
	...
	_normalizeData(options = {}) {
		...
		const profNormalize = this.profiler.begin('@ _normalizeData');
		let result = super._normalizeData(options);
		profNormalize.end();

		const profMerge = this.profiler.begin('> merge');
