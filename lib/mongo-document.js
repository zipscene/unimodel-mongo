...
class MongoDocument extends SchemaDocument {
	...
	/**
	 * Expand data components into a "map field" object, containing a `mapPath` and `keys` properties.
	 *
	 * @method _expandMapFields
	 * @private
	 ...
	 * @return {Object} With properties `mapPath` (path to get a field value) and
	 *   `keys` (map keys found along this path).
	 */
	_expandMapFields(data, path, components) {
		let component = _.first(components);
		let subcomponents = _.rest(components);
		...
		if (subcomponents.length) {
			if (subschema.type === 'map') {
				for (let key in subdata) {
					let childFields = this._expandMapFields(subdata[key], `${subpath}.$`, subcomponents);
					for (let { mapPath, keys } of childFields) {
						mapFields.push({
							mapPath: `${component}.${key}.${mapPath}`,
							keys: [ key ].concat(keys)
						});
					}
				}
			} else {
				let childFields = this._expandMapFields(subdata, subpath, subcomponents);
				for (let { mapPath, keys } of childFields) {
					mapFields.push({
						mapPath: `${component}.${mapPath}`,
						keys
					});
				}
			}
	...
	/**
	 * Get indexed fields associated with 'maps' in this document, which are converted to arrays of
	 * stringified BSON arrays, containing map keys, and field values of the index.
	 *
	 * @method _getIndexedMapFields
	 * @private
	 * @return {Object}
	 * @since v0.4.0
	 */
	_getIndexedMapFields(data) {
		let mapIndicies = [];
		// Find all fields we need to extract into BSON arrays
		for (let index of this.model.getIndices()) {
			for (let field in index.spec) {
				if (/^_mapidx_/.test(field)) {
					mapIndicies.push(field);
				}
			}
		}
		mapIndicies = _.uniq(mapIndicies);

		let indexedMapFields = {};
		for (let mapIndex of mapIndicies) {
			// Strip off the '_mapidx_'
			let mapPathsStr = mapIndex.substring(8);
			// Split on '_' field identifier
			let [ path, ...fields ] = mapPathsStr.split('_');
			let components = path.split('|');
			let mapFields = this._expandMapFields(data, '', components);
			for (let { mapPath, keys } of mapFields) {
				let indexData = objtools.deepCopy(keys);
				let allFound = true;
				//TODO: support sparse indexes, and non-field indexes
				for (let field of fields) {
					let mapPathField = (mapPath ? `${mapPath}.` : '') + field;
					let value = objtools.getPath(data, mapPathField);
				...
				if (allFound) {
					...
					if (!indexedMapFields[mapIndex]) { indexedMapFields[mapIndex] = []; }
					indexedMapFields[mapIndex].push(BSON.serialize(indexData).toString());
				}
			}
		}

		return indexedMapFields;
	}
	...
	save() {
		...
		return this.model.collectionPromise
			...
			.then(() => {
				...
				objtools.merge(normalizedData, this._getIndexedMapFields(normalizedData));
			})
