...
class MongoDocument extends SchemaDocument {
	...
	constructor(model, data, options = {}) {
		...
		const profPrepare = this.profiler.begin('MongoDocument#constructor @ _prepareNormalizeData');
		...
		if (this.options.isExisting) {
			const profCopy = this.profiler.begin('MongoDocument#constructor > deepCopy');
		...
		const profTrigger = this.profiler.begin('MongoDocument#constructor > triggerSync:post-init');
	...
	_prepareNormalizeData(data) {
		...
		for (let field in data) {
			if (_.startsWith(field, '_mapidx_')) {
	...
	_normalizeData(options = {}) {
		...
		const profPrepare = this.profiler.begin('MongoDocument#_normalizeData @ _prepareNormalizeData');
		let fields = this._prepareNormalizeData(data);
		profPrepare.end();

		const profNormalize = this.profiler.begin('MongoDocument#_normalizeData @ _normalizeData');
		let result = super._normalizeData(options);
		profNormalize.end();

		const profMerge = this.profiler.begin('MongoDocument#_normalizeData > merge');
	...
	save(options = {}) {
		...
		let prof = this.profiler.begin('#save');
