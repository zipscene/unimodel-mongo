...
class MongoDocument extends SchemaDocument {
	...
	constructor(model, data, options = {}) {
		let origData = objtools.deepCopy(data);
		...
		if (this.options.isExisting) {
			this._originalMongoData = origData;
			if (!model.explicitInternalId) delete this._originalMongoData._id;
		}
	...
	_prepareNormalizeData(data) {
		...
		if (data._id !== undefined) {
			fields._id = data._id;
			if (!this.model.explicitInternalId) delete data._id;
		}
	 ...
	 */
	getInternalId() {
		if (this.model.explicitInternalId) {
	...
	getOriginalUniqueId() {
		let field = this.model.options.uniqueIdField;
		if (field === '_id') return this._originalId;
		if (!this._originalData) return undefined;
		return this._originalData[field];
	}
	...
	save(options = {}) {
		...
		return this.model.collectionPromise
			...
			.then(() => {
				...
				let origId = this.getOriginalUniqueId();

				// If no id exists, save as a new document
				if (!this._originalData || typeof _id === 'undefined') {
				...
				if (_id !== this._originalId || id !== origId) {
				...
				let update = commonQuery.Update.createFromDiff(this._originalMongoData, mongoDocumentData);
				...
				const handleRevConflict = async(whichUpdate, searchedRev) => {
					if (!MongoDocument._debugRev) {
						throw new XError(
							XError.CONFLICT,
					...
					let cursor = collection.find(generalQuery, { readPreference: 'primary', limit: 1 });
					let results = await cursor.toArray();
					let actualRev = '<documentnotfound>';
					if (results.length) {
						actualRev = results[0].__rev;
					}
					throw new XError(
						XError.CONFLICT,
						`The document was updated elsewhere.  Update op ${whichUpdate}.  Looking for rev ${searchedRev}.  Found rev ${actualRev}.  Query ${JSON.stringify(query)}`
					);
				};

				return collection[updateFn]({
					[this.model.options.uniqueIdField]: id,
					__rev: this._revisionNumber
				}, update)
					.then((result) => {
						if (result.result.nModified < 1) {
							return handleRevConflict('UpdateA', this._revisionNumber);
							//throw new XError(
							//	XError.CONFLICT,
							//	'The document was updated elsewhere.'
							//);
			...
			.then(() => {
				...
				if (!this.model.explicitInternalId) delete this._originalMongoData._id;
		...
		return this.model.collectionPromise
			...
			.then(() => {
				return collection.removeOne({
					...
					[this.model.options.uniqueIdField]: this.getOriginalUniqueId(),
					__rev: this._revisionNumber
				});
			})
