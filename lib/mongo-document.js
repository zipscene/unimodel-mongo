	...
	save() {
		...
		return this.model.collectionPromise
			...
			.then(() => {
				...
				this.data = this.model.schema.normalize(this.data);
			})
			...
			.then(() => {
				...
				newData = objtools.deepCopy(this.data);
				// Set the revision
				this.__rev = this._revisionNumber;
				// Add the indexed map arrays to the normalized data
				this.model.normalizeDocumentIndexedMapValues(newData);
			})
			.then(() => {
				...
				let update = commonQuery.Update.createFromDiff(this._originalData, newData);
			...
			.then(() => {
				...
				this._revisionNumber = newData.__rev;
				if (newData._id) {
					this.setInternalId(newData._id);
					this._originalId = newData._id;
				}
