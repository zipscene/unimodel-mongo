...
class MongoDb extends EventEmitter {
	...
	/**
	 * Sends a kill command to any in-progress operations with a top-level
	 * $comment field matching the provided string.
	 *
	 * @method killOpsWithComment
	 * @param {String} comment
	 * @return {Promise} - Resolves when the operation is complete.
	 */
	killOpsWithComment(comment) {
		return this._getOpIdsWithComment(comment)
			.then((opIds) => this._killOps(opIds));
	}

	/**
	 * Searches through the results of a currentOp command to find the
	 * opIds of any in-progress operations with the provided comment.
	 *
