...
const { query: { Query } } = require('zs-common-query');
 ...
 * @return {Object} - A MongoDB aggregate expression
 */
exports.makeDiscreteValueExpression = function(schema, field) {
...
exports.makeGroupExpression = function(schema, aggregate) {
	...
	let groupValueSpecs = groupBy.map((groupBySpec) => {
		...
		if (groupBySpec.ranges) {
			...
			valueExpr = this.makeDiscreteValueExpression(schema, field);
		}
				...
				if (groupBy) {
					...
					for (let keyIndex = 0; keyIndex < groupBy.length; keyIndex++) {
						...
						if (key.timeComponent) {
							...
							let subschema = schema.getSubschemaData(key.field);
							if (subschema && subschema.type === 'boolean') keyValue = !!keyValue;
						}
