 ...
 * @return {Object} - A MongoDB aggregate expression
 */
exports.makeDiscreteValueExpression = function(schema, field, groupBySpec) {
...
exports.makeGroupExpression = function(schema, aggregate) {
	...
	let groupValueSpecs = groupBy.map((groupBySpec) => {
		...
		if (groupBySpec.ranges) {
			...
			valueExpr = this.makeDiscreteValueExpression(schema, field, groupBySpec);
		}
...
exports.createAggregatePipeline = function(schema, query, aggregates, options = {}) {
		...
		groupExpressionData.specIndexes.forEach((index) => {
			...
			if (aggregateSpec.stats) {
				_.forEach(aggregateSpec.stats, (stat, field) => {
					...
					if (stat.sum) fields[`agg_${index}_sum_${fieldNum}`] = { $sum: `$${field}` };
					if (stat.stddev) fields[`agg_${index}_stddev_${fieldNum}`] = { $stdDevPop: `$${field}` };
 ...
 * @since v0.1.0
 */
exports.createAggregateResult = function(schema, pipelineData, aggregates) {
